                ...
                <div className='text-[#F3F4F5]  pt-7 md:pt-2 grid justify-between  items-center'>
                    <div className='text-left w-full sm:w-3/5'>
                        {distribution.map((item, index) => {
                            const { id, name, mainDescription, description, points, subPoints, details } = item
                            return (
                                <div>
                                            ...
                                            {details?.map((item,index) => {
                                                return (
                                                    <div className={` ${index === details.length - 1 ? '' : 'py-2'}`} key={index}>
                                                        <h2 className={`font-GilroyBold text-xs pb-5'}`}>{item.title}</h2>
                                                        <p className='font-GilroyRegular text-xs '>{item.desc}</p>
                                                    </div>
                                                )
                                            })}
